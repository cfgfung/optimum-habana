...
class GaudiDDPMPipeline(GaudiDiffusionPipeline, DDPMPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not hasattr(self, 'remainings_unet'):
            self.remainings_unet = copy.deepcopy(self.unet)
            logger.info('Prepared a Unet for the remaining calculations. Timesteps is excluded in the Unet')

        if self.use_hpu_graphs:
           #self.unet = self.ht.hpu.wrap_in_hpu_graph(self.unet, disable_tensor_cache=True)
           self.remainings_unet = self.ht.hpu.wrap_in_hpu_graph(self.remainings_unet, disable_tensor_cache=True)
        ...
        if self.use_habana:
            ...
            self.remainings_unet = self.remainings_unet.to(self._device)
        ...
        for i in self.progress_bar(num_inference_steps):
            ...
            with torch.autocast(device_type="hpu", dtype=torch.bfloat16, enabled=self.gaudi_config.use_torch_autocast):
                #model_output = self.unet(image, timestep, False, emb).sample # --- this is calling the original unet and that's why it has something in the class labels --emb
                model_output = self.remainings_unet(image, timestep, False, emb).sample
